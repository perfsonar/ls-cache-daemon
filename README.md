...
The Lookup Service Cache Daemon periodically downloads a tarball of CSV files using a conditional HTTP GET containing a listing of perfSONAR Services. This code does NOT generate the CSV files, it simply handles downloading them periodically. These files are used to then populate the Toolkit GUI's test selection interface.

##Getting the Code
You may checkout the code with the following command:

```
git clone --recursive https://github.com/perfsonar/ls-cache-daemon.git
```
